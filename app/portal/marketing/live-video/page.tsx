  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-xl font-semibold text-gray-900">Live & Video</h1>
      </div>

      <div className="bg-white border border-gray-200 rounded-lg p-4">
        <div className="text-sm font-semibold text-gray-800 mb-3">Schedule Live Session</div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg">
        <div className="px-4 py-3 border-b border-gray-200 text-sm font-medium text-gray-700">Sessions</div>
        {loading ? (
          <div className="p-6 text-sm text-gray-500">Loading sessions...</div>
        ) : campaigns.length === 0 ? (
          <div className="p-6 text-sm text-gray-500">No live sessions scheduled.</div>
        ) : (
            ...
            {campaigns.map((c) => (
              <div key={c.id} className="p-4 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-gray-500">Scheduled: {c.start_at || "—"}</div>
                </div>
                <button
